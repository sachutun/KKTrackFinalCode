...
/* connection = DriverManager.getConnection("jdbc:mysql://localhost:8889/KKTrack","root","root"); */
    ...
    String nbl = request.getParameter("nbl"+code);
  ...
  String s3= "UPDATE `NewInventory` SET `Location`="+nbl+" WHERE Code="+code+" AND Branch="+branch;
 /*  System.out.println(s3); */

     ps2 = conn.prepareStatement("UPDATE `NewInventory` SET `Location`=? WHERE Code=? AND Branch=?");
    ps2.setString(1,nbl);
    ps2.setString(2,code);
    ps2.setString(3,branch);
    ps2.executeUpdate(); 

    	  response.sendRedirect("binlocupdate.jsp?res=1"); 
  //  }
}catch (Exception e) {
    	 e.printStackTrace();
    	 }
